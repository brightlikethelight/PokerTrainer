...
// Mock the repository
jest.mock('../../infrastructure/HandHistoryRepository');
...
describe('HandHistoryService', () => {
  ...
  beforeEach(() => {
    ...
    mockRepository = {
      ...
      saveHand: jest.fn(),
      getHandsBySession: jest.fn(),
      getAnalytics: jest.fn(),
    };

    // Replace the service repository with our mock
    HandHistoryService.repository = mockRepository;
  ...
  describe('Session Management', () => {
    test('should start a session with default configuration', async () => {
      ...
      mockRepository.createSession.mockResolvedValue(sessionId);
    ...
    test('should start a session with custom configuration', async () => {
      ...
      expect(mockRepository.createSession).toHaveBeenCalledWith(customConfig);
      expect(result).toBe(sessionId);
    });
    ...
    test('should end a session successfully', async () => {
      ...
      mockRepository.getHandsBySession.mockResolvedValue([]);
    ...
    test('should handle session end failure', async () => {
      ...
      HandHistoryService.currentSession = sessionId;
      mockRepository.updateSession.mockRejectedValue(error);
  ...
  describe('Hand Completion', () => {
    ...
    test('should complete hand with winner correctly', async () => {
      ...
      mockRepository.saveHand.mockResolvedValue(handId);
      ...
      expect(mockRepository.saveHand).toHaveBeenCalledWith(HandHistoryService.currentHand);
    ...
    test('should not complete hand without active hand', async () => {
      ...
      expect(mockRepository.saveHand).not.toHaveBeenCalled();
    });
    ...
    test('should handle hand completion failure', async () => {
      ...
      mockRepository.saveHand.mockRejectedValue(error);
  ...
  describe('Session Statistics', () => {
    ...
    test('should calculate session statistics correctly', async () => {
      ...
      mockRepository.getHandsBySession.mockResolvedValue(hands);
    ...
    test('should handle session stats calculation failure', async () => {
      ...
      mockRepository.getHandsBySession.mockRejectedValue(error);
  ...
  describe('Integration Scenarios', () => {
    test('should handle complete hand flow', async () => {
      // Start session
      mockRepository.createSession.mockResolvedValue('session-1');
      ...
      mockRepository.saveHand.mockResolvedValue('hand-1');
    ...
    test('should handle multiple concurrent hands', async () => {
      mockRepository.createSession.mockResolvedValue('session-multi');
      await HandHistoryService.startSession();
    ...
    test('should maintain data integrity across session', async () => {
      // Start session
      mockRepository.createSession.mockResolvedValue('integrity-session');
      await HandHistoryService.startSession();
      ...
      for (let i = 0; i < 5; i++) {
        ...
        mockRepository.saveHand.mockResolvedValue(`hand-${i + 1}`);
