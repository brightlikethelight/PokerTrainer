...
import HandHistoryRepository from '../infrastructure/HandHistoryRepository';
...
class HandHistoryService {
  ...
  constructor(repository = null) {
    this.repository = repository || new HandHistoryRepository();
  ...
  async startSession(sessionData = {}) {
    try {
      ...
      const sessionId = await this.repository.createSession(sessionInfo);
  ...
  async endSession() {
    ...
    try {
      ...
      await this.repository.updateSession(this.currentSession, {
        endTime: Date.now(),
  ...
  async completeHand(gameState, winners, showdown = false) {
    ...
    try {
      ...
      // Save to repository
      const handId = await this.repository.saveHand(this.currentHand);
  ...
  async getSessionStats() {
    ...
    try {
      const hands = await this.repository.getHandsBySession(this.currentSession);
